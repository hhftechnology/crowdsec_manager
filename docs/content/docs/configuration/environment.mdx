...
| `CACHE_MAX_ENTRIES` | `512` | Max entries retained in the shared in-memory TTL cache |
...
| Variable | Default | Purpose |
|---|---|---|
| `ENABLE_PROFILING` | `false` | Enable `/api/debug/runtime` and `/api/debug/pprof/*` diagnostics |
| `PROFILING_ALLOW_REMOTE` | `false` | Allow diagnostics from non-loopback clients when profiling is enabled |
