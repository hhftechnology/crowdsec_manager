...
/**
 * AppHeader - Top navigation header component
 * Provides breadcrumbs, user menu, and contextual actions
 * with enhanced responsive behavior and accessibility
 * following the shadcn-admin template architecture
 */
export function AppHeader({ 
  ...
  const { isMobile: hookIsMobile, needsTouchOptimization } = useBreakpoints()
  ...
  return (
    <header className={cn(
      ...
      // Responsive height and padding with touch-friendly sizing
      isMobile ? (needsTouchOptimization ? "h-16 px-4" : "h-14 px-4") : "h-16 px-6",
      className
    )}>
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              className={cn(
                ...
                needsTouchOptimization ? "h-10 w-10" : "h-9 w-9"
              )}
            ...
            {/* Mobile Logo */}
            <div className="flex items-center gap-2" role="img" aria-label="CrowdSec Manager">
                      ...
                      <BreadcrumbItem>
                        {index === breadcrumbs.length - 1 ? (
                          ...
                          <BreadcrumbLink 
                            href={item.href}
                            className={cn(
                              "truncate focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 rounded",
                              isMobile && "max-w-[80px]"
                            )}
                          >
                            {item.label}
      ...
      {/* Right Section: External Links & User Menu */}
      <div className="flex items-center gap-2 sm:gap-4">
        {/* External Links - Hidden on mobile, shown on tablet+ */}
        <div className={cn(
          ...
          <Button 
            ...
            asChild 
            className={cn(
              "gap-2 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
              needsTouchOptimization && "h-10"
            )}
          >
            <a
          ...
          <DropdownMenuTrigger asChild>
            <Button
              ...
              className={cn(
                ...
                isMobile ? (needsTouchOptimization ? "h-10 w-10" : "h-8 w-8") : "h-9 w-9"
              )}
              aria-label={`User menu for ${user?.name || "CrowdSec user"}`}
            >
              <Avatar className={cn(
                "cursor-pointer hover:opacity-80 transition-opacity bg-transparent",
                isMobile ? (needsTouchOptimization ? "h-9 w-9" : "h-7 w-7") : "h-8 w-8"
              )}>
                {user?.avatar ? (
