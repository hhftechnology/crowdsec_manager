...
function buildDraft(
  ...
  return normalizeConnectionProfileDraft(
    profile ?? createDefaultConnectionProfileDraft(),
  );
}
...
export default function MorePage() {
  ...
  const reconnect = async () => {
    ...
    try {
      ...
      if (ok) {
        showActionSuccess(
          'Connected',
          normalizeConnectionProfileDraft(draft).baseUrl,
        );
      }
    } catch (err) {
  ...
  return (
    <div className="pb-nav">
      <PageHeader
        title="Settings"
        subtitle="Connection, proxy access, and theme"
      />
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-4">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Update the saved connection profile used for API requests and
              terminal WebSockets.
            </p>
          </div>
          ...
          <p className="text-xs text-muted-foreground">
            Current server: {connectionProfile?.baseUrl || 'Not connected'}
          </p>
        </section>
