...
function createDraft(
  overrides: Partial<ConnectionProfileDraft> = {},
...
function Harness({ draft }: { draft: ConnectionProfileDraft }) {
  ...
  return (
    <div>
      <button type="button" onClick={() => void login(draft)}>
        Login
      </button>
...
describe('ApiContext', () => {
  ...
  it('rejects explicit http urls when insecure mode is off', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('error')).toHaveTextContent(
        'HTTPS is required in secure mode',
      );
    });
  ...
  it('falls back from https to http when insecure mode is enabled', async () => {
    const fetchSpy = vi
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            success: true,
            data: { containers: [], allRunning: true },
          }),
          {
            status: 200,
    ...
    render(
      <ApiProvider>
        <Harness
          draft={createDraft({ baseUrl: '10.0.0.1:8080', allowInsecure: true })}
        />
      </ApiProvider>,
    );
    ...
    expect(
      JSON.parse(localStorage.getItem('csm_connection_profile') || '{}'),
    ).toMatchObject({
