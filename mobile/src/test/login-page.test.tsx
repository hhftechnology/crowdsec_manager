...
describe('LoginPage', () => {
  ...
  it('shows proxy fields only in proxy mode and blocks incomplete proxy submissions', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Server URL'), { target: { value: 'proxy.example.com' } });
    fireEvent.change(screen.getByLabelText('Proxy username'), { target: { value: 'alice' } });
    fireEvent.change(screen.getByLabelText('Proxy password'), { target: { value: 'secret' } });
  ...
  it('shows pangolin fields and keeps connect disabled until token is provided', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Pangolin access token')).toBeInTheDocument();
    });
    expect(screen.queryByLabelText('Token query parameter')).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Connect' })).toBeDisabled();

    fireEvent.click(screen.getByRole('button', { name: 'Show' }));
    expect(screen.getByLabelText('Token query parameter')).toBeInTheDocument();

    fireEvent.change(screen.getByLabelText('Pangolin URL'), { target: { value: 'pangolin.example.com' } });
    expect(screen.getByRole('button', { name: 'Connect' })).toBeDisabled();

    fireEvent.change(screen.getByLabelText('Pangolin access token'), { target: { value: 'pp6evkhe.3kyqq4a7eay6rp6ow6dacallhm' } });
    expect(screen.getByRole('button', { name: 'Connect' })).toBeEnabled();
  });
});
