...
describe('ApiClient', () => {
  ...
  it('adds basic auth headers for proxy mode', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({ Authorization: 'Basic YWxpY2U6c2VjcmV0' }),
      }),
    );
  });
  ...
  it('adds Pangolin access token headers on every HTTP request', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      'https://api.example.com/api/test?custom_token=pp6evkhe.3kyqq4a7eay6rp6ow6dacallhm',
      expect.objectContaining({
        method: 'GET',
  ...
  it('embeds proxy credentials in websocket urls', async () => {
    const client = new ApiClient(createProfile({
      mode: 'proxy-basic',
      proxyUsername: 'alice',
      proxyPassword: 'secret',
    ...
    await expect(client.getWebSocketUrl('/api/terminal/crowdsec')).resolves.toBe(
      'wss://alice:secret@api.example.com/api/terminal/crowdsec',
    );
  });

  it('adds Pangolin access token query params to websocket urls', async () => {
    const client = new ApiClient(createProfile({
      mode: 'pangolin',
      pangolinToken: 'pp6evkhe.3kyqq4a7eay6rp6ow6dacallhm',
      pangolinTokenParam: 'custom_token',
